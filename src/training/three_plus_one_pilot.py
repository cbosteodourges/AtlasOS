...
class ThreePlusOnePilotPlanner:
    ...
    def _stimulus_week(self, monday: date, number: int, wellness: str) -> PilotWeek:
        ...
        sessions = [
            ...
            PilotSession(
                monday + timedelta(days=1), "vo2_short", vo2_title,
                48 + (number - 1) * 2,
                specific_minutes=vo2_planned, specific_minutes_min=vo2_min,
                specific_minutes_max=vo2_max, repetitions_min=vo2_reps_min,
                repetitions_max=vo2_reps_max, is_specific=True, is_metabolic=True,
                instructions=["Régularité prioritaire ; aucune répétition sprintée.", *autoregulation],
            ),
            PilotSession(
                monday + timedelta(days=3), "threshold_short", threshold_title,
                50 + (number - 1) * 2,
                specific_minutes=threshold_planned, specific_minutes_min=threshold_min,
                specific_minutes_max=threshold_max, repetitions_min=threshold_reps_min,
                repetitions_max=threshold_reps_max, is_specific=True, is_metabolic=True,
                instructions=["Rester autour ou légèrement sous le SV2 longitudinal.", *autoregulation],
            ),
            PilotSession(
            ...
            PilotSession(
                monday + timedelta(days=6), "recovery_cycling",
                "Vélo de récupération", 75,
