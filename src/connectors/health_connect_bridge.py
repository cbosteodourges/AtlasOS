...
class HealthConnectBridge:
    ...
    def _activity(cls,item:dict[str,Any])->NormalizedActivity:
        ...
        return NormalizedActivity(provider="health_connect",external_id=str(item["source_id"]),activity_type=activity_type,start_time=str(item["start_time"]),duration_seconds=float(item.get("duration_seconds",0)),distance_meters=item.get("distance_meters"),calories_kcal=item.get("calories_kcal"),average_heart_rate_bpm=item.get("average_heart_rate_bpm"),maximum_heart_rate_bpm=item.get("maximum_heart_rate_bpm"),average_speed_mps=item.get("average_speed_mps"),elevation_gain_m=item.get("elevation_gain_m"),source_device=item.get("source_device"),samples=samples,raw_metadata={"health_connect":True,"health_connect_exercise_type":raw_type,"health_connect_local_day":item.get("local_day"),"lap_count":item.get("lap_count",0),"segment_count":item.get("segment_count",0),"laps":[x for x in item.get("laps",[]) if isinstance(x,dict)],"health_connect_segments":[x for x in item.get("segments",[]) if isinstance(x,dict)],"health_connect_data_coverage":item.get("data_coverage",{}),"average_cadence":item.get("average_cadence_spm"),"maximum_cadence":item.get("maximum_cadence_spm"),"average_power":item.get("average_power_watts"),"maximum_power":item.get("maximum_power_watts")})
