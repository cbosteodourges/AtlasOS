...
## Nouvel aperçu : Santé retenue, navigation complète et calendrier d’origine

Christophe valide la direction visuelle du module Santé avec ses trois rubriques : Charge & contraintes, Douleurs & ressenti, Prévention. Il souhaite la reprendre dans Atlas. Il demande également les accès Aujourd’hui, Profil, **Plan d’entraînement** (libellé complet), Santé et Connexions & données, et exige de conserver la présentation du plan de l’application d’origine sur PC, tablette et mobile. Cette précision remplace la liste simplifiée du précédent aperçu pour le plan.

Livrable : `prototype/atlas-apercu-complet-2026-09-10.html`, ouvert sur Santé. Sur mobile, les quatre accès principaux sont suivis d’un accès Connexions & données sur toute la largeur ; aucun libellé n’est réduit à une icône. Les avatars, jauges et profils de l’aperçu mixte sont conservés.

Le calendrier n’est pas redessiné : l’aperçu embarque le module `app/js/atlas-training-calendar.js`, les styles `app/css/performance-running.css` et `app/css/atlas-responsive.css`, ainsi que le panneau et les styles intégrés de `app/performance-running.html`. Il conserve les semaines dépliables, la grille des grands écrans, les jours sélectionnables sur mobile, les cartes colorées, les fiches, leurs étapes et leur chronologie. Les fichiers de l’application active ne sont pas modifiés.

Le script `scripts/build_atlas_complete_preview.py` reconstruit le fichier autonome à partir de ces sources. Il remplace seulement les quatre références d’image du module embarqué par l’avatar existant inclus, ajuste le cadre extérieur au nouvel aperçu et fournit une adaptation isolée : date de démonstration fixe, stockage en mémoire, réponses fictives aux lectures et refus des écritures vers le moteur. Le calendrier est placé dans un cadre isolé, sans accès au stockage de l’application ou au réseau. Une navigation entre les modules masque son cadre sans le recréer. Le programme de démonstration contient 10 semaines et 30 séances inventées ; **aucun programme personnel ni compte rendu réel n’est récupéré dans le fichier public**. Les rapports sans données affichent l’état vide du module. Les adaptations, imports, recalculs et changements de calendrier réels nécessitent l’application active ; la synchronisation de la rubrique Connexions est explicitement une simulation.

Vérifications réalisées : syntaxe JavaScript des deux documents, identifiants HTML statiques uniques, exécution des fonctions de rendu originales sur les 10 semaines/30 fiches et des 70 sélecteurs de jours, cohérence des durées des chronologies fictives, fonctions de rendu des six vues et des dialogues de connexions avec un DOM simulé. Les sources actives et les trois aperçus précédents sont inchangés. Il ne s’agit pas d’un test navigateur ni d’une validation sur appareil réel.

La direction visuelle Santé est retenue pour l’intégration. Ce lot livre le nouvel aperçu demandé ; il ne prétend pas avoir raccordé ses formulaires au stockage réel ni activé de nouvelles règles biomécaniques ou d’adaptation. Les corrections du moteur relevées dans l’audit et la vérification de la sauvegarde privée Windows restent des travaux distincts. Référence avant ce lot : `8284522b922a2df4fa91f5f51edb39126c155b78`.
