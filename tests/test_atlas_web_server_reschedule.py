...
class AtlasWebServerRescheduleTests(unittest.TestCase):
    ...
    def test_server_and_calendar_expose_reschedule_route(self):
        ...
        self.assertIn('data-chart-detail-range', calendar)
        self.assertIn('compact-interval-details" open', calendar)
        self.assertIn('phase-detail-row', calendar)
        self.assertIn('data-phase-filter="recovery"', calendar)
        self.assertIn('pas votre niveau de forme', calendar)
        self.assertNotIn('class="execution-score-explanation"', calendar)
        ...
        self.assertIn('Récupérations ·', calendar)
        self.assertIn('/api/atlas-coach/recalculate-execution', server)
        self.assertIn('data-recalculate-execution', calendar)
        self.assertIn('/api/atlas-coach/workout-export', server)
        self.assertIn('data-garmin-fit-export', calendar)
        self.assertIn('Fiabilité et calcul du compte-rendu', calendar)
